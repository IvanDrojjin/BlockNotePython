...
from tabulate import tabulate  # tabulate 0.9.0
from os import path
import csv
...
def read_file():
    ...
    while True:
        if path.exists(db_file):
            ...
            ask_user = input('Хотите создать новый файл (y/n): ')
            if ask_user.lower() == 'y':
...
def search_by_id():
    read_file()
    search = input('Введите ID искомой записи: ')
    if search == '':
        ui.show_error_input_msg()
    else:
        result = searching(id_note=search)
        if len(result) == 0:
            print('Ничего не найдено(нет ID).')
        else:
...
def search_by_date():
    read_file()
    search = input('Введите дату создания(изменения) записи для поиска\n'
                   '(формат YYYY-MM-DD, YYYY - год, MM - месяц, DD - день, например 2020-01-01): ')
...
def change_note():
    ...
    if ch_id_ind == 0:
        ...
        if ask_user.lower() == 'y':
            ...
            print(f'Запись с ID {id_num} была изменена(успешно)!')
            input(ui.waiting_msg)
        elif ask_user.lower() == 'n':
            print(f'Запись с ID {id_num} не изменена !')
            input(ui.waiting_msg)
        else:
            log.logging.error('Неправильный выбор.')
            ui.show_error_input_msg()
...
def del_note():
    ...
    if del_id_index == 0:
        ...
        ask_user = input("Подтвердите ударение (y/n): ")
        if ask_user.lower() == 'y':
            note_date = datetime.now().strftime("%Y-%m-%d %H:%M")
            notebook[del_id_index] = [str(id_num), '', f'<Deleted {note_date}>', '']
            log.logging.info(f'Deleted note with id {del_id_index}')
            write_file(notebook)
            print(f'Запись with ID {id_num} была удалена(успешно) !')
            log.logging.info('Database was changed')
            input(ui.waiting_msg)
        elif ask_user.lower() == 'n':
            print(f'Запись с ID {id_num} не была удалена!')
            input(ui.waiting_msg)
        else:
            log.logging.error('Incorrect selection')
            ui.show_error_input_msg()
